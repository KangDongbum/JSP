...
public class Logger {
	 ...
	 * @param req
	 */
	public static void log(ServletRequest req) {
		/**
		 * 1. 요청 메소드
		 * 2. 요청 URL
		 * 3. 접속 IP
		...
		if(req instanceof HttpServletRequest) {
			...
			String referer =req2.getHeader("referer"); // 유입경로
			
			sb.append(method);
			sb.append(" / URL : ");
			sb.append(req2.getRequestURL());
			sb.append(" / IP : ");
			sb.append(req2.getRemoteAddr());
			if(referer != null) {
				sb.append(" / REF : ");
				sb.append(referer);
			}
			sb.append(" / UA : "); // User Agent(브라우저 접속 정보)
			sb.append(req2.getHeader("user-agent"));
			sb.append(" / LANG : "); // 사용 언어
			sb.append(req2.getHeader("accept-language"));
